    ...
    Mat& R, Mat& t)
{
    ...
    p1 = Point3f(Vec3f(p1) / N); // cam1's mid point 
    p2 = Point3f(Vec3f(p2) / N); // cam2's mid point 
...
void bundleAdjustment(
    ...
    std::unique_ptr<g2o::BlockSolverX> solver_ptr = g2o::make_unique<g2o::BlockSolverX>(std::move(linear_solver));
    //g2o::OptimizationAlgorithm* algorithm = new g2o::OptimizationAlgorithmLevenberg(std::move(block_solver));
...
int main(int argc, char** argv)
{
    ...
    Mat depth1 = imread(argv[3], IMREAD_UNCHANGED);  // 16-bit, single-channel
    Mat depth2 = imread(argv[3], IMREAD_UNCHANGED);  // 16-bit, single-channel
    ...
    for (DMatch m : matches)
    {
        ...
        float dd1 = float(d1) / 5000.0; // 5000 ???
